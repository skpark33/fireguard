...
CSmtp::CSmtp(std::string strConfigPath, std::string strCompany)
{
	//ciDEBUG(5, ("CSmtp(%s, %s)", strConfigPath.c_str(), strCompany.c_str()) );

	m_strConfigPath = strConfigPath;

	//http를 이용한 웹페이지에 post 방식으로 보내는 경우
	//자체 SMTP 서버를 구축하고 해당하는 웹페이지를 만든 경우임
	//USE_HTTP
	CSecurityIni iniSec;
	...
	int port = atoi(iniSec.ReadValue(m_strCompany.c_str(), "SMTP_PORT", m_strConfigPath.c_str()));
	if(port <= 0){
		// port 가 0 이면, 결국, INI 에 셋팅된 값이 없다는 뜻이다.  
		// 이경우 UBC_TT.ini 에 SMTP_<enterprise> 가 셋팅되어있지 않다는 뜻이므로,  
		// Default인 [SMTP] 엔트리를 쓰도록 한다.
		m_strCompany = "SMTP";
	}

	m_bUseHttp = atoi(iniSec.ReadValue(m_strCompany.c_str(), "USE_HTTP", m_strConfigPath.c_str()));
...
/// @remarks
/// 단일 수신자에게 메일을 발송한다. \n
/// @param (const) char* szRecipient : (in) 수신자 이메일 주소
/// @param (const) char* szTitle : (in) 이메일의 제목
/// @param (std::string) strText : (in) 이메일의 본문
/// @return <형: bool> \n
///			<true: 성공> \n
///			<false: 실패> \n
/////////////////////////////////////////////////////////////////////////////////
bool CSmtp::Send(const char* szRecipient, const char* szTitle, std::string strText)
{
	CSecurityIni iniSec;
	if(!m_bUseHttp)
	{
		std::string strSMTPServer, strSenderID, strSenderPWD, strSenderName, strSenderAddr, strReplyAddr;
		int nSMTPPort, nSecurityType;
		
		//SMTP_SERVER
		strSMTPServer = iniSec.ReadValue(m_strCompany.c_str(), "SMTP_SERVER", m_strConfigPath.c_str());

		//SMTP_PORT
		nSMTPPort = atoi(iniSec.ReadValue(m_strCompany.c_str(), "SMTP_PORT", m_strConfigPath.c_str()));

		//SECURITY_TYPE
		nSecurityType = atoi(iniSec.ReadValue(m_strCompany.c_str(), "SECURITY_TYPE", m_strConfigPath.c_str()));

		//SMTP_ID
		strSenderID = iniSec.ReadValue(m_strCompany.c_str(), "SMTP_ID", m_strConfigPath.c_str());

		//SMTP_PWD
		strSenderPWD = iniSec.ReadValue(m_strCompany.c_str(), "SMTP_PWD", m_strConfigPath.c_str());

		//SENDER_NAME
		strSenderName = iniSec.ReadValue(m_strCompany.c_str(), "SENDER_NAME", m_strConfigPath.c_str());

		//SENDER_ADDR
		strSenderAddr = iniSec.ReadValue(m_strCompany.c_str(), "SENDER_ADDR", m_strConfigPath.c_str());

		//REPLY_ADDR
		strReplyAddr = iniSec.ReadValue(m_strCompany.c_str(), "REPLY_ADDR", m_strConfigPath.c_str());

		//ciDEBUG(1,("SMTP_SERVER=%s",strSMTPServer.c_str()));
		//ciDEBUG(1,("SMTP_PORT=%d",nSMTPPort));
		//ciDEBUG(1,("SECURITY_TYPE=%d",nSecurityType));
		//ciDEBUG(1,("SMTP_ID=%s",strSenderID.c_str()));
		//ciDEBUG(1,("SMTP_PWD=%s",strSenderPWD.c_str()));
		//ciDEBUG(1,("SENDER_NAME=%s",strSenderName.c_str()));
		//ciDEBUG(1,("SENDER_ADDR=%s",strSenderAddr.c_str()));
		//ciDEBUG(1,("REPLY_ADDR=%s",strReplyAddr.c_str()));

		if(strSMTPServer == "" || nSMTPPort == 0 || strSenderID == "" || strSenderPWD == ""
			|| strSenderName == "" || strSenderAddr == "" || szRecipient == "")
		{
			//ciDEBUG(1, ("Emoty some input value"));
			return false;
		}//if

		SetSMTPServer(strSMTPServer.c_str(), nSMTPPort);
		SetSecurityType((CSmtp::security_type)nSecurityType);

		SetLogin(strSenderID.c_str());					//SMTP ID
		SetPassword(strSenderPWD.c_str());				//SMTP Password
		SetSenderName(strSenderName.c_str());			//Sender name	
		SetSenderMail(strSenderAddr.c_str());			//Sender address
		SetReplyTo(strReplyAddr.c_str());				//Reply address
		SetSubject(szTitle);							//Title
		AddRecipient(szRecipient);						//Receiver address
		...
		catch(ECSmtp e)
		{
			...
			{
				//ciDEBUG(1, ("Fail to send mail : %s", strError.c_str()));
				return false;
			}//if
		}//try
	}
	...
	{
		//std::string strURL, strCompany, strSenderAddr;
		//
		////HTTP_URL
		//strURL = iniSec.ReadValue(m_strCompany.c_str(), "HTTP_URL", m_strConfigPath.c_str());

		////COMPANY
		//strCompany = iniSec.ReadValue(m_strCompany.c_str(), "COMPANY", m_strConfigPath.c_str());
		...
		//Post로 URL에 보낸다.
		//[기아]
		//http://211.232.57.219:8081/manager/smtpresult.aspx 에 POST 로 다음의 인자에 값을 넣어 던져주시면 됩니다.
		//company      :   KIA
		//send_email   : sss@ss.ss.ss
		//recive_mail  : sss@ss.ss.ss,sss@ss.ss.ss,sss@ss.ss.ss   (콤마로구분) 
		//title        : 제목입니다.
		//contents     : 내용입니다.
		//*********************************************************************************************/
		//CString strRet;
		...
		//
		//strSendMsg = "company=";
		//strSendMsg += strCompany.c_str();
		//strSendMsg += "&send_email=";
		//strSendMsg += strSenderAddr.c_str();
		//strSendMsg += "&receive_mail=";
		//strSendMsg += szRecipient;
		//strSendMsg += "&title=";
		//strSendMsg += szTitle;
		//strSendMsg += "&contents=";
		//strSendMsg += strText.c_str();

		//TRACE(strSendMsg.c_str());

		////UTF-8 로 변환
		//string strUtf8 = AnsiToUTF8(strSendMsg);

		//CHttpRequest http_request;
		//if(!http_request.RequestPost(strURL.c_str(), strUtf8.c_str(), strRet))
		//{
		//	//ciDEBUG(1, ("Fail to send mail : %s", strRet));
		//	return false;
		//}
		//else
		//{
		//	//ciDEBUG(1, ("Success send mail : %s", strRet));
		//}//if
	}//if
...
/// @remarks
/// 여러 수신자에게 메일을 발송한다. \n
/// @param (std::list<string>) listRecipient : (in) 수신자 이메일 주소 리스트
/// @param (const) char* szTitle : (in) 이메일의 제목
/// @param (std::string) strText : (in) 이메일의 본문
/// @return <형: bool> \n
///			<true: 성공> \n
...
/////////////////////////////////////////////////////////////////////////////////
/////////////////////////////////////////////////////////////////////////////////
bool CSmtp::Send(std::list<string> listRecipient, const char* szTitle, std::string strText)
{
	CSecurityIni iniSec;
	if(!m_bUseHttp)
	{
		...
		for(std::list<string>::iterator it = listRecipient.begin(); it != listRecipient.end(); ++it)
		{
			strRecipient = (*it);
			AddRecipient(strRecipient.c_str());		//Receiver address
		}//for
		...
		catch(ECSmtp e)
		{
			...
			{
				//ciDEBUG(1, ("Fail to send mail : %s", strError));
				return false;
			}//if
		}//try
	}
	else
	{
		...
		//{
		//	strTmp = (*it);
		//	strRecipient += strTmp.c_str();
		//	strRecipient += ",";
		//}//for
		//strRecipient.TrimRight(",");
...
///			변환된 UTF-8 문자열 \n
/////////////////////////////////////////////////////////////////////////////////
string CSmtp::AnsiToUTF8(string& strAnsi)
{
	/*
	WCHAR unicode[15000]; 
    char utf8[15000];
	...
	*/
	...
	int nLen = strAnsi.length();
	WCHAR* pUniCode = (WCHAR*)malloc(sizeof(WCHAR)*nLen*2);
	char* pUtf8 = (char*)malloc(sizeof(char)*nLen*2);

	memset(pUniCode, 0x00, sizeof(WCHAR)*nLen*2);
	memset(pUtf8, 0x00, sizeof(char)*nLen*2);

	::MultiByteToWideChar(CP_ACP, 0, strAnsi.c_str(), -1, pUniCode, sizeof(WCHAR)*nLen*2); 
    ::WideCharToMultiByte(CP_UTF8, 0, pUniCode, -1, pUtf8, sizeof(char)*nLen*2, NULL, NULL); 

	string strReturn(pUtf8);
