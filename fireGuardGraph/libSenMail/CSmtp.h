...
{
public:
	CSmtp(std::string strConfigPath, std::string strCompany = "");
	virtual ~CSmtp();

	//메일전송 함수
	bool Send(const char* szRecipient, const char* szTitle, std::string strText);			///<단일 수신자에게 메일을 발송한다.
	bool Send(std::list<string> listRecipient, const char* szTitle, std::string strText);	///<여러 수신자에게 메일을 발송한다.

	string AnsiToUTF8(string& strAnsi);														///<Ansi 문자열을 UTF-8로 변환한다.
	
	void AddAttachFile(const char *path) { AddAttachment(path); }  
...
private:
	...
	std::string m_strConfigPath;			//ConfigPath
	std::string m_strCompany;				//기업 구분
