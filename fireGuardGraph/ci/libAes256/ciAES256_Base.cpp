...
ciS_Box< false > const ciAES256_Base::s_sbox;
ciS_Box< true  > const ciAES256_Base::s_inv_sbox;
...
ciAES256_Base::ciAES256_Base( std::string const & key )
{
    set_key( key );
}
...
/*---------------------------------------------*
 * Constructor with key given as a ciByte_Block<32> object
 *---------------------------------------------*/

ciAES256_Base::ciAES256_Base( ciByte_Block< 32 > const & key )
{
	init( key );
}
...
/*---------------------------------------------*
 * Returns the key (as a ciByte_Block<32> object)
 *---------------------------------------------*/

void
ciAES256_Base::set_key( std::string const & key )
{
    if ( key.size( ) < 32 )
...
ciByte_Block< 16 > &
ciAES256_Base::encrypt( ciByte_Block< 16 > & buf )
{
    ciByte_Block< 32 > key = m_enc_key;
    ...
    for ( byte i = 1; i < 14; ++i )
    {
        ...
        if ( i & 1 )
            add_round_key( buf, ciByte_Block< 16 >( key, 16 ) );
        else
        {
...
ciByte_Block< 16 >
ciAES256_Base::encrypt( ciByte_Block< 16 > const & buf )
{
    ciByte_Block< 16 > res( buf );
    return encrypt( res );
}
...
ciByte_Block< 16 > &
ciAES256_Base::decrypt( ciByte_Block< 16 > & buf )
{
    ciByte_Block< 32 > key = m_dec_key;
...
ciByte_Block< 16 >
ciAES256_Base::decrypt( ciByte_Block< 16 > const & buf )
{
    ciByte_Block< 16 > res( buf );
    return decrypt( res );
}
...
void
ciAES256_Base::expand_enc_key( ciByte_Block< 32 > & key,
                             byte             & rcon ) 
{
...
void
ciAES256_Base::expand_dec_key( ciByte_Block< 32 > & key,
                             byte             & rcon ) 
{
    for ( int i = 31; i >= 20; --i )
        key[ i ] ^= key[ i - 4 ];
...
void
ciAES256_Base::sub_bytes( ciByte_Block< 16 > & buf )
{
    for ( int i = 0; i < 16; ++i )
        buf[ i ] = s_sbox[ buf[ i ] ];
}
...
void
ciAES256_Base::inv_sub_bytes( ciByte_Block< 16 > & buf )
{
    for ( int i = 0; i < 16; ++i )
...
void
ciAES256_Base::add_round_key( ciByte_Block< 16 >       & buf,
...
void
ciAES256_Base::shift_rows( ciByte_Block< 16 > & buf )
{
    byte tmp;
...
void
ciAES256_Base::inv_shift_rows( ciByte_Block< 16 > & buf )
{
    byte tmp;
...
void
ciAES256_Base::mix_columns( ciByte_Block< 16 > & buf )
{
    byte * b = buf;
...
void
ciAES256_Base::inv_mix_columns( ciByte_Block< 16 > & buf )
{
    byte * b = buf;
...
void
ciAES256_Base::init( ciByte_Block< 32 > const & key )
{
	m_enc_key = key;
	m_dec_key = key;
 ...
 * it compile with '-DTEST_ciAES256_BASE'.
 *
 ...
 * with '-DTEST_ciAES256_BASE_FIPS'.
 *---------------------------------------------*/

#if defined TEST_ciAES256_BASE
...
main( )
{
	...
	ciAES256_Base aes( key );

	ciByte_Block< 16 > block( txt );
...
#if defined TEST_ciAES256_BASE_FIPS
...
main( )
{
    ...
    std::cout << " -> "
              << ( res == ciByte_Block< 16 >( rd ) ? "Success" : "Failure" )
              << std::endl;
