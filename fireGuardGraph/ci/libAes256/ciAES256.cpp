...
ciAES256::ciAES256( std::string const & key,
...
void
ciAES256::set_chaining_mode( Chaining_Mode mode )
{
	m_mode = mode;
	...
	switch ( mode )
	{
		case ECB :
			enc = &ciAES256::ecb;
			dec = &ciAES256::ecb_inv;
            m_use_padding = true;
			break;

		case CBC :
			enc = &ciAES256::cbc;
			dec = &ciAES256::cbc_inv;
            m_use_padding = true;
			break;

		case PCBC :
			enc = &ciAES256::pcbc;
			dec = &ciAES256::pcbc_inv;
            m_use_padding = true;
			break;

		case CFB128 :
			enc = &ciAES256::cfb128;
			dec = &ciAES256::cfb128_inv;
            m_use_padding = false;
			break;

		case CFB8 :
			enc = &ciAES256::cfb8;
			dec = &ciAES256::cfb8_inv;
            m_use_padding = false;
			break;

		case OFB :
			enc = dec = &ciAES256::ofb;
            m_use_padding = false;
			break;

		case CTR :
			enc = dec = &ciAES256::ctr;
            m_use_padding = false;
			break;
...
void
ciAES256::set_padding_mode( Padding_Mode mode )
{
    ciByte_Block< 16 >::set_padding_mode( mode );
}
...
void
ciAES256::set_IV( std::string const & IV )
{
    if ( ! IV.empty( ) )
    {
        if ( IV.size( ) < 16 )
...
std::string
ciAES256::encrypt( std::string const & in,
...
std::ostream &
ciAES256::encrypt( std::istream & in,
                 std::ostream & out,
                 bool           no_padding_block )
{
...
std::string
ciAES256::decrypt( std::string const & in,
                 bool                no_padding_block )
{
	size_t len = in.size( );
    bool bad_len = false;
    ...
    switch ( m_mode )
    {
        ...
        default :
            bad_len = (int) len < ( no_padding_block ? 16 : 32 ) || (int) len % 16;
    }
	...
	std::string out;
    for ( int i = start; i < (int) len; i += 16 )
    {
		ciByte_Block< 16 > buf( in, i );
...
std::ostream &
ciAES256::decrypt( std::istream & in,
                 std::ostream & out,
                 bool           no_padding_block )
{
    // Make sure streams can read from and written to
    ...
    while ( in.read( buf, 16 ) )
    {
        ...
        if ( m_use_padding && ! no_padding_block && in.peek( ) == EOF )
        {
            ...
            out.write( tmp.data( ), (int) tmp.size( ) );
        }
        else
...
ciByte_Block< 16 > &
ciAES256::ecb( ciByte_Block< 16 > & buf )
{
...
ciByte_Block< 16 > &
ciAES256::ecb_inv( ciByte_Block< 16 > & buf )
{
...
ciByte_Block< 16 > &
ciAES256::cbc( ciByte_Block< 16 > & buf )
{
	buf ^= m_IV;
...
ciByte_Block< 16 > &
ciAES256::cbc_inv( ciByte_Block< 16 > & buf )
{
    ciByte_Block< 16 > tmp( buf );
...
ciByte_Block< 16 > &
ciAES256::pcbc( ciByte_Block< 16 > & buf )
{
    ciByte_Block< 16 > tmp( buf );
...
ciByte_Block< 16 > &
ciAES256::pcbc_inv( ciByte_Block< 16 > & buf )
{
    ciByte_Block< 16 > tmp( buf );
...
ciByte_Block< 16 > &
ciAES256::cfb128( ciByte_Block< 16 > & buf )
{
...
ciByte_Block< 16 > &
ciAES256::cfb128_inv( ciByte_Block< 16 > & buf )
{
	ciByte_Block< 16 > tmp( m_IV );
...
ciByte_Block< 16 > &
ciAES256::cfb8( ciByte_Block< 16 > & buf )
{
...
ciByte_Block< 16 > &
ciAES256::cfb8_inv( ciByte_Block< 16 > & buf )
{
	ciByte_Block< 16 > shift_reg( m_IV );
    ...
    for ( int i = 0; i < buf.init_len( ); ++i )
    {
        ciByte_Block< 16 > stmp( shift_reg );
...
ciByte_Block< 16 > &
ciAES256::ofb( ciByte_Block< 16 > & buf )
{
	m_aes256_base. encrypt( m_IV );
	return buf ^= m_IV;
}
...
ciByte_Block< 16 > &
ciAES256::ctr( ciByte_Block< 16 > & buf )
{
	ciByte_Block< 16 > tmp = m_IV++;
	m_aes256_base. encrypt( tmp );
	return buf ^= tmp;
 ...
 * Test main() functions:
 *
 * a) Test string encryption on all modes: compile with -DTEST_ciAES256_STRING
 * b) Test stream encryption on all modes: compile with -DTEST_ciAES256_STREAM
 * c) Test encryption of (README.txt) file:  compile with -DTEST_ciAES256_FILE
 *
 * Note that this isn't a real test, it just gives some feedback when
 * something is obviously badly broken.
 *---------------------------------------------*/

#if defined TEST_ciAES256_STRING
...
main( )
{
    ...
    aes.set_padding_mode( ciAES256::ANSIX9_23 );

	for ( int mode = ciAES256::ECB; mode <= ciAES256::CTR; ++mode )
	{
		aes.set_chaining_mode( static_cast< ciAES256::Chaining_Mode >( mode ) );
...
#if defined TEST_ciAES256_STREAM
