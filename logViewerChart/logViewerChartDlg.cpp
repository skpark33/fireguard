// LogViewerChartDlg.cpp : implementation file
//
...
// Constructor
//
CLogViewerChartDlg::CLogViewerChartDlg(CWnd* pParent /*=NULL*/)
    : CDialog(CLogViewerChartDlg::IDD, pParent)
{
    m_hIcon = AfxGetApp()->LoadIcon(IDR_MAINFRAME);
    m_ranSeries = 0;
	_cameraIdx = 0;
	//_thresholdMax = 100.0f;
	//_thresholdMin = -20.0f;
	m_dataSeries = 0;
	m_timeStamps = 0;
}
...
// Destructor
//
CLogViewerChartDlg::~CLogViewerChartDlg()
{
    delete m_ranSeries;
...
void CLogViewerChartDlg::DoDataExchange(CDataExchange* pDX)
{
...
BEGIN_MESSAGE_MAP(CLogViewerChartDlg, CDialog)
    ON_WM_PAINT()
    ON_WM_QUERYDRAGICON()
	...
	ON_BN_CLICKED(IDC_LOG_FOLDER, &CLogViewerChartDlg::OnBnClickedLogFolder)
	ON_BN_CLICKED(IDC_SHOW_ANOTHER, &CLogViewerChartDlg::OnBnClickedShowAnother)
	ON_WM_CLOSE()
	ON_BN_CLICKED(IDC_BT_CLOSE, &CLogViewerChartDlg::OnBnClickedBtClose)
END_MESSAGE_MAP()

/////////////////////////////////////////////////////////////////////////////
// CLogViewerChartDlg message handlers

//
// Initialization
//
BOOL CLogViewerChartDlg::OnInitDialog()
{
	...
	DrawAll();
	TraceLog(("OnInitDialog Complete"))
	return TRUE;

}

void CLogViewerChartDlg::DrawAll()
{
    // Load the data
    loadData();
...
void CLogViewerChartDlg::OnPaint()
{
    if (IsIconic())
    {
...
HCURSOR CLogViewerChartDlg::OnQueryDragIcon()
{
...
//
void CLogViewerChartDlg::OnPointerPB()
{
...
//
void CLogViewerChartDlg::OnZoomInPB()
{
...
//
void CLogViewerChartDlg::OnZoomOutPB()
{
...
//
void CLogViewerChartDlg::OnSavePB()
{
...
//
void CLogViewerChartDlg::OnViewPortChanged()
{
...
//
void CLogViewerChartDlg::OnMouseMovePlotArea()
{
...
/////////////////////////////////////////////////////////////////////////////
// CLogViewerChartDlg methods

//
// Load the data
//
void CLogViewerChartDlg::loadData()
{
	CString iniPath = UBC_CONFIG_PATH;
	iniPath += UBCBRW_INI;
	char buf[512];
	//memset(buf, 0x00, 512);
	//GetPrivateProfileString("FIRE_WATCH", "MAX_THRESHOLD", "100.0", buf, 512, iniPath);
	//_thresholdMax = atof(buf);

	//memset(buf, 0x00, 512);
	//GetPrivateProfileString("FIRE_WATCH", "MIN_THRESHOLD", "-20.0", buf, 512, iniPath);
	//_thresholdMin = atof(buf);
	...
	_logger.SetFullPath(_logFile );
	int maxCount = _logger.LoadLog(_cameraIdx);
	if (maxCount == 0)
	{
		TraceLog(("GetLog(%s) failed", _logFile));
		return;
	}
	TraceLog(("max %d log founded", maxCount));

    // In this example, we just use random numbers as data.
	//m_timeStamps = m_ranSeries->getDateSeries(1827, Chart::chartTime(2021, 1, 1), 86400);
	m_timeStamps = _logger.GetTime();
	m_dataSeries = _logger.GetLog();
	//m_minSeries = m_ranSeries->getSeries(m_dataSeries->len, _thresholdMin, _thresholdMin);
	//m_maxSeries= m_ranSeries->getSeries(m_dataSeries->len, _thresholdMax, _thresholdMax);
	TraceLog(("Data Load Complete"));
...
// Initialize the CChartViewer 
//
void CLogViewerChartDlg::initChartViewer(CChartViewer *viewer)
{
...
//
void CLogViewerChartDlg::drawChart(CChartViewer *viewer)
{
	...
	DoubleArray viewPortDataSeries[3];
	
	viewPortDataSeries[1] = DoubleArray(m_dataSeries->data + startIndex, noOfPoints);
	//viewPortDataSeries[0] = DoubleArray(m_minSeries.data + startIndex, noOfPoints);
	//viewPortDataSeries[2] = DoubleArray(m_maxSeries.data + startIndex, noOfPoints);
	...
	int color[MAX_CAMERA] = { 0xffa800, 0xfff000, 0x30ff00, 0x007323, 0x00f0ff, 0x2764ff, 0x0a0572, 0x9600ff };
	
	CString name;
	name.Format("Camera%d", _cameraIdx);
	layer->addDataSet(viewPortDataSeries[1], color[_cameraIdx], name);
	//layer->addDataSet(viewPortDataSeries[0], 0xff0000, "MAX");
	//layer->addDataSet(viewPortDataSeries[2], 0xff0000, "MIN");
...
//
void CLogViewerChartDlg::drawFullChart(CViewPortControl *vpc, CChartViewer *viewer)
{
	...
	layer->addDataSet(*(m_dataSeries), color[_cameraIdx]);
...
//
void CLogViewerChartDlg::trackLineLegend(XYChart *c, int mouseX)
{
...
//
void CLogViewerChartDlg::loadButtonIcon(int buttonId, int iconId, int width, int height)
{
...
void CLogViewerChartDlg::OnBnClickedLogFolder()
{
	// 내문서
...
void CLogViewerChartDlg::OnBnClickedShowAnother()
{
	CString filter = "FireWatch Log Files (*.csv)|*.csv||";

	CFileDialog dlg(TRUE, NULL, UBC_UPLOADER_PATH, OFN_FILEMUSTEXIST | OFN_HIDEREADONLY, filter, this);
	if (dlg.DoModal() == IDOK)
	{
		_logFile =  dlg.GetFileName();
...
void CLogViewerChartDlg::OnClose()
{
...
void CLogViewerChartDlg::OnBnClickedBtClose()
{
	_logger.destroy();
